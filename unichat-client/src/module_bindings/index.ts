...
export class RemoteReducers {
  ...
  onRegisterUser(callback: (ctx: ReducerEventContext, name: string) => void) {
    this.connection.onReducer("RegisterUser", callback);
  }

  removeOnRegisterUser(callback: (ctx: ReducerEventContext, name: string) => void) {
    this.connection.offReducer("RegisterUser", callback);
  }

  sendMessage(to: Identity, content: string) {
    const __args = { to, content };
    let __writer = new BinaryWriter(1024);
  ...
  onSendMessage(callback: (ctx: ReducerEventContext, to: Identity, content: string) => void) {
    this.connection.onReducer("SendMessage", callback);
  }

  removeOnSendMessage(callback: (ctx: ReducerEventContext, to: Identity, content: string) => void) {
